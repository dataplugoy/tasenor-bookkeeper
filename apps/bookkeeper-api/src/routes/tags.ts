import express from 'express'
import data from '../lib/data'
import knex from '../lib/knex'
import { tasenor } from '../lib/middleware'
import { encode } from 'base64-arraybuffer'

const router = express.Router()

const placeholder = () => {
  return {
    mime: 'image/png',
    picture: Buffer.from('', 'base64')
  }
}

router.get('/',
  ...tasenor({ json: true, url: true }),
  async (req, res, next) => {
    await data.listAll(await knex.db(res.locals.user, res.locals.db), 'tags').then(data => {
      data = data.map(p => ({ ...p, picture: p.picture ? encode(p.picture) : null }))
      res.send(data)
    }).catch(next)
  }
)

router.post('/',
  ...tasenor({ json: true, url: true, upload: true }),
  async (req, res, next) => {
    let tag = req.body

    const old = await (await knex.db(res.locals.user, res.locals.db))('tags').select('id').where({ tag: tag.tag }).first()
    if (old) {
      return res.status(400).send({ message: 'Tag already exists.' })
    }

    if (tag.picture && tag.mime) {
      tag.picture = Buffer.from(tag.picture, 'base64')
    } else {
      tag = { ...tag, ...placeholder() }
    }
    return data.createOne(await knex.db(res.locals.user, res.locals.db), 'tags', tag)
      .then(data => res.send(data))
      .catch(next)
  }
)

router.get('/:id',
  ...tasenor({ json: true, url: true }),
  async (req, res, next) => {
    data.getOne(await knex.db(res.locals.user, res.locals.db), 'tags', parseInt(req.params.id))
      .then(tag => {
        if (!tag) {
          return res.status(404).send({ message: 'No such tag.' })
        }
        tag.picture = tag.picture ? encode(tag.picture) : null
        res.send(tag)
      })
      .catch(next)
  }
)

router.patch('/:id',
  ...tasenor({ json: true, url: true, upload: true }),
  async (req, res, next) => {
    const obj = req.body
    if (obj.picture) {
      obj.picture = Buffer.from(obj.picture, 'base64')
    }
    data.updateOne(await knex.db(res.locals.user, res.locals.db), 'tags', parseInt(req.params.id), obj)
      .then(() => res.status(204).send())
      .catch(next)
  }
)

router.delete('/:id',
  ...tasenor({ json: true, url: true }),
  async (req, res, next) => {
    const id = parseInt(req.params.id)
    await data.deleteOne(await knex.db(res.locals.user, res.locals.db), 'tags', id).catch(next)
    res.sendStatus(204)
  }
)

router.get('/:id/view',
  async (req, res, next) => {
    const id = parseInt(req.params.id)
    if (isNaN(id)) {
      return res.status(404).send({ message: 'Invalid tag ID.' })
    }
    data.getOne(await knex.db(res.locals.user, res.locals.db), 'tags', id)
      .then(tag => {
        res.header('Content-Type', tag.mime)
        res.send(tag.picture)
      })
      .catch(next)
  }
)

export default router
