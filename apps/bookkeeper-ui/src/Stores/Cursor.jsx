...
class Cursor {
  ...
  constructor(store) {
    ...
    document.addEventListener('keydown', (event) => {
      ...
      let keyName = (
        ...
        (event.metaKey ? 'Meta+' : '') +
        event.key)
      // Unify result for system specific shortcut keys.
      keyName = keyName.replace(Configuration.COMMAND_KEY_MOD, 'Command')
      keyName = keyName.replace('Ctrl', 'Icon')
      // Handle.
  ...
  addModal(name, modal = {}) {
    ...
    this.activeModal.push({ ...modal, name })
  }
  ...
  */
  ...
  handle(key) {
    ...
    if (!result && this.activeModal.length) {
      const n = this.activeModal.length - 1
      const modal = this.activeModal[n]
      if (!modal[fn]) {
        return null
      }
      result = modal[fn](this, key)
      if (result && KEY_DEBUG) {
  ...
  /**
   * Disable focus change using tab-key on non-empty topologies.
   */
  keyTab() {
    const topology = this.getTopology()
    if (!topology || topology.length === 0 || topology[0].length === 0) {
      return
    }
