import { AccountNumber, Language, PluginCode, ReportID, ReportLine, ReportOptions, Version, ID } from '@tasenor/common'
import { ReportPlugin } from '@tasenor/common-node'
import dayjs from 'dayjs'
import quarterOfYear from 'dayjs/plugin/quarterOfYear'
dayjs.extend(quarterOfYear)

class FinnishBalanceSheetReportInvestment extends ReportPlugin {
  constructor() {
    super('balance-sheet-investment-detailed' as ReportID, 'balance-sheet-investment' as ReportID)

    this.schemes = new Set(['FinnishInvestmentCompany'])

    this.code = 'FinnishBalanceSheetReportInvestment' as PluginCode
    this.id = 29 as ID
    this.title = 'Balance Sheet Report (Finnish - Investment)'
    ...
    this.use = 'backend'
    this.type = 'report'
    this.description = 'Investment company balance sheet report translated in Finnish.'

    this.languages = {
      en: {
        'report-balance-sheet-investment-detailed': 'Detailed balance sheet',
        'report-balance-sheet-investment': 'Balance sheet'
      },
      fi: {
        'report-balance-sheet-investment-detailed': 'Tase tilierittelyin',
        'report-balance-sheet-investment': 'Tase'
      }
    }
  }
...
export default FinnishBalanceSheetReportInvestment
