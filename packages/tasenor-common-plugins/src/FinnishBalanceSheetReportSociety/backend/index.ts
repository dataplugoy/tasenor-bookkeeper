import { AccountNumber, Language, PluginCode, ReportID, ReportOptions, Version, ID } from '@tasenor/common'
import { ReportPlugin } from '@tasenor/common-node'
import dayjs from 'dayjs'
import quarterOfYear from 'dayjs/plugin/quarterOfYear'
dayjs.extend(quarterOfYear)

class FinnishBalanceSheetReportSociety extends ReportPlugin {
  constructor() {
    super('balance-sheet-society' as ReportID, 'balance-sheet-society-detailed' as ReportID)

    this.schemes = new Set(['FinnishSociety'])

    this.code = 'FinnishBalanceSheetReportSociety' as PluginCode
    this.id = 39 as ID
    this.title = 'Balance Sheet Report (Finnish - Society)'
    ...
    this.releaseDate = '2025-08-12'
    this.use = 'backend'
    this.type = 'report'
    this.description = 'Balance sheet report for Finnish societies, i.e. rekisteröity yhdistys.'

    this.languages = {
      en: {
        'report-balance-sheet-society-detailed': 'Detailed alance sheet',
        'report-balance-sheet-society': 'Balance sheet'
      },
      fi: {
        'report-balance-sheet-society-detailed': 'Tase tilierittelyin',
        'report-balance-sheet-society': 'Tase'
      }
    }
  }
...
export default FinnishBalanceSheetReportSociety
