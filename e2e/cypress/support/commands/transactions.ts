...
/**
 * Select a transaction by its description text.
 */
Cypress.Commands.add('selectNamedTx', (text: string) => {
  cy.contains(text).click()
  cy.xpath(SELECTED_DATE).should('exist')
})

/**
 * Verify that the account balance matches.
 */
Cypress.Commands.add('accountBalanceShouldBe', (account: string, balance: string) => {
  cy.xpath(`//tr[contains(@class, "BalanceLine")][./td[contains(@class, "number")][text()="${account}"]][./td[contains(@class, "balance")]/*[text()="${balance}"]]`).should('exist')
})

/**
 * Wait until the transaction sub-selection shows text.
 */
Cypress.Commands.add('waitUntilTxSelected', (text: string) => {
  cy.xpath(`//*[contains(@class, "TransactionDetails")][contains(@class, "account")][contains(@class, "sub-selected")]//*[text()='${text}']`).should('exist')
})

/**
 * Enter edit mode and type text in the current transaction cell.
 */
Cypress.Commands.add('editTransactionCell', (text: string) => {
  cy.realPress('Enter')
  cy.realType(text)
  cy.realPress('Enter')
})
...
/**
 * Clear the content of the current transaction cell.
 */
Cypress.Commands.add('clearTransactionCell', () => {
