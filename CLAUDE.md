...
- **`apps/bookkeeper/`** — React 17 + Vite frontend. MobX for state, MUI 5 for components, React Router 6 for routing. The bundled plugins are scanned into `src/Plugins/index.json` + `index.jsx` at build time (see `bin/initialize-plugins.mjs`).
...
Plugins extend both UI and backend. They cover importers (bank/crypto), reports (balance sheet, income statement), company types, and utilities. All plugins are a **permanent, bundled part of the repository** — they live in `packages/tasenor-common-plugins/` and are symlinked into each app's plugin directory (`apps/bookkeeper/src/Plugins/` and `apps/bookkeeper-api/src/plugins/`). There is no dynamic install/update/fetch of plugin repositories: every bundled plugin is always available on the server, and per-company activation is handled via subscriptions. The `index.json` catalog is generated by scanning the bundled plugins at build/boot (`plugins.updatePluginList()` in `@tasenor/common-node`).
